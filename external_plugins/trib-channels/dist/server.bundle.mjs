...
`);
    ...
    this.client.on("messageCreate", (msg) => {
      ...
      this.handleInbound(msg).catch(
        (e) => process.stderr.write(`trib-channels discord: handleInbound failed: ${e}
`)
      );
    });
...
`);
  ...
  async downloadAttachment(chatId, messageId) {
    ...
    for (const att of msg.attachments.values()) {
      const path3 = await this.downloadSingleAttachment(att);
      results.push({
        path: path3,
...
`);
  ...
  async downloadSingleAttachment(att) {
    ...
    const path3 = join(this.inboxDir, `${Date.now()}-${att.id}.${ext}`);
...
function createBackend(config2) {
  if (config2.backend !== "discord") {
    process.stderr.write(`trib-channels: unsupported backend "${config2.backend}" (discord only)
`);
    process.exit(1);
  }
  if (!config2.discord?.token) {
...
function tryRead(path3) {
  try {
    return readFileSync3(path3, "utf8").trim();
  } catch {
    return null;
  }
}
...
// lib/scheduler.ts
import { readFileSync as readFileSync5, writeFileSync as writeFileSync4, unlinkSync } from "fs";
...
var NOPLUGIN_DIR = join4(tmpdir(), "trib-channels-noplugin");
var EVENT_LOG = join4(DATA_DIR, "event.log");
function logEvent(msg) {
  const line = `[${(/* @__PURE__ */ new Date()).toISOString()}] ${msg}
`;
  try {
    process.stderr.write(`trib-channels event: ${msg}
`);
  } catch {
  }
  try {
...
${prompt}`;
  ...
  static SCHEDULER_LOCK = join6(tmpdir2(), "trib-channels-scheduler.lock");
  start() {
    ...
    ensureNopluginDir();
    try {
      writeFileSync4(_Scheduler.SCHEDULER_LOCK, `${process.pid}
${Date.now()}`, { flag: "wx" });
    } catch (err) {
      if (err.code === "EEXIST") {
        try {
...
`);
  ...
  tick() {
    this.tickAsync().catch(
      (err) => process.stderr.write(`trib-channels scheduler: tick error: ${err}
`)
    );
  }
  async tickAsync() {
...
`);
      ...
      this.fireTimed(s, type).catch(
        (err) => process.stderr.write(`trib-channels scheduler: ${s.name} failed: ${err}
`)
      );
    }
  ...
  tickProactive(now, dateStr) {
    ...
    const elapsed = (Date.now() - this.proactiveLastFire) / 6e4;
        ...
        if (execMode === "script") {
          ...
          if (scriptResult && this.sendFn) {
            await this.sendFn(channelId2, scriptResult).catch(
              (err) => process.stderr.write(`trib-channels scheduler: ${schedule.name} relay failed: ${err}
`)
            );
          }
...
`);
    ...
    if (this.proactive?.feedback) {
      const feedbackPath = join6(DATA_DIR, "proactive-feedback.md");
...
`);
    ...
    this.lastFired.set(`proactive:${item.topic}`, (/* @__PURE__ */ new Date()).toISOString().slice(0, 16));
...
`;
  try {
    process.stderr.write(`trib-channels webhook: ${msg}
`);
  } catch {
  }
  try {
...
// lib/event-pipeline.ts
import { basename } from "path";

// lib/event-queue.ts
import { readdirSync as readdirSync2, readFileSync as readFileSync7, writeFileSync as writeFileSync6, renameSync as renameSync3 } from "fs";
import { join as join8 } from "path";

// lib/state-file.ts
import { mkdirSync as mkdirSync4, readFileSync as readFileSync6, renameSync as renameSync2, unlinkSync as unlinkSync2, writeFileSync as writeFileSync5 } from "fs";
...
function writeTextFile(filePath, value) {
  ...
  writeFileSync5(filePath, value);
}
function writeJsonFile(filePath, value) {
  const tmpPath = filePath + ".tmp";
  ensureDir(dirname(filePath));
  writeFileSync5(tmpPath, JSON.stringify(value));
...
var EventQueue = class {
  ...
  start() {
    ...
    const tickMs = (this.config.tickInterval ?? 60) * 1e3;
  ...
  enqueue(item) {
    ...
    writeFileSync6(join8(QUEUE_DIR, filename), JSON.stringify(item, null, 2));
...
${item.prompt}
  ...
  readItem(file) {
    try {
      return JSON.parse(readFileSync7(join8(QUEUE_DIR, file), "utf8"));
    } catch (err) {
      if (err.code !== "ENOENT") {
...
// lib/output-forwarder.ts
import { readFileSync as readFileSync8, readdirSync as readdirSync3, existsSync as existsSync4, statSync as statSync2, watch, openSync, readSync, closeSync } from "fs";
...
function readSessionRecord(pid) {
  ...
  try {
    const session = JSON.parse(readFileSync8(sessionFile, "utf8"));
...
var OutputForwarder = class _OutputForwarder {
  ...
  setContext(channelId, transcriptPath, options = {}) {
    ...
    try {
      const fileSize = options.replayFromStart ? 0 : existsSync4(this.transcriptPath) ? statSync2(this.transcriptPath).size : 0;
  ...
  readNewLines() {
    if (!this.transcriptPath || !existsSync4(this.transcriptPath)) {
      return { lines: [], nextFileSize: this.readFileSize };
    }
    let fd = null;
    try {
      const stat = statSync2(this.transcriptPath);
...
`);
  ...
  static isHidden(name) {
    ...
    if (name.includes("plugin_trib-channels") && !name.endsWith("recall_memory") || name === "reply" || name === "react" || name === "edit_message" || name === "fetch" || name === "download_attachment") return true;
    return false;
  }
...
// lib/session-control.ts
import { existsSync as existsSync5, readFileSync as readFileSync10, unlinkSync as unlinkSync3, writeFileSync as writeFileSync8 } from "fs";
import { setTimeout as delay } from "timers/promises";

// lib/runtime-paths.ts
import { readFileSync as readFileSync9, readdirSync as readdirSync4, statSync as statSync3, writeFileSync as writeFileSync7 } from "fs";
import { execFileSync as execFileSync2 } from "child_process";
import { tmpdir as tmpdir3 } from "os";
import { join as join10 } from "path";
var RUNTIME_ROOT = join10(tmpdir3(), "trib-channels");
...
function looksLikeTribChannelsServer(pid) {
  ...
  try {
    ...
    return cmd.includes("trib-channels") || pluginRoot && cmd.includes(pluginRoot) || cmd.includes("tsx server.ts") || cmd.includes("node") && cmd.includes("server");
  } catch {
    return false;
  }
}
...
function killAllPreviousServers() {
  try {
    const oldPid = parseInt(readFileSync9(SERVER_PID_FILE, "utf8").trim(), 10);
...
function writeServerPid() {
  ensureRuntimeDirs();
  writeFileSync7(SERVER_PID_FILE, String(process.pid));
}
function clearServerPid() {
  try {
    const current = readFileSync9(SERVER_PID_FILE, "utf8").trim();
...
function cleanupStaleRuntimeFiles(now = Date.now()) {
  ...
  forEachFile(RUNTIME_ROOT, (fullPath, file) => {
    ...
    try {
      if (now - statSync3(fullPath).mtimeMs > RUNTIME_STALE_TTL) removeFileIfExists(fullPath);
    } catch {
    }
  });
...
async function controlClaudeSession(instanceId, command, timeoutMs = 3e3) {
  ...
  try {
    unlinkSync3(responsePath);
  } catch {
  }
  writeFileSync8(controlPath, JSON.stringify({ id, command, requestedAt: Date.now() }));
  const started = Date.now();
  while (Date.now() - started < timeoutMs) {
    if (existsSync5(responsePath)) {
      try {
        const payload = JSON.parse(readFileSync10(responsePath, "utf8"));
...
// lib/memory-client.mjs
import http2 from "node:http";
import fs from "node:fs";
import os from "node:os";
import path from "node:path";
var PORT_FILE = path.join(os.tmpdir(), "trib-memory", "memory-port");
...
function memoryFetch(method, endpoint, body = null) {
  return new Promise((resolve3, reject) => {
    ...
    req.on("timeout", () => {
      req.destroy();
      reject(new Error("memory-service timeout"));
    });
    if (payload) req.write(payload);
    req.end();
  });
}
async function appendEpisode(data) {
  try {
...
async function getHints(query, options = {}) {
  try {
...
async function ingestTranscript(filePath) {
  try {
...
function buildModalRequestSpec(customId, pending2, profile2) {
  switch (customId) {
    case "sched_add_next": {
      const fields = [
    ...
    case "profile_edit":
      return {
        ...
        title: "Edit Profile",
        fields: [
          { id: "name", label: "Name", required: false, value: profile2.name ?? "" },
          { id: "role", label: "Role", required: false, value: profile2.role ?? "" },
          { id: "lang", label: "Language (ko / en / ja / zh)", required: false, value: profile2.lang ?? "" },
          { id: "tone", label: "Tone", required: false, value: profile2.tone ?? "" }
        ]
      };
    default:
      return null;
...
function logCrash(label, err) {
  ...
  if (err instanceof Error && err.message.includes("EPIPE")) {
    try {
      const crashLog = path2.join(DATA_DIR, "crash.log");
      fs2.appendFileSync(crashLog, `[${localTimestamp()}] trib-channels: EPIPE detected, disconnecting + exiting
`);
    } catch {
    }
    process.exit(1);
...
`;
  ...
  try {
    const crashLog = path2.join(DATA_DIR, "crash.log");
    fs2.appendFileSync(crashLog, msg);
  } catch {
  }
}
...
var _bootLogEarly = path2.join(
  process.env.CLAUDE_PLUGIN_DATA || path2.join(os2.tmpdir(), "trib-channels"),
  "boot.log"
);
fs2.appendFileSync(_bootLogEarly, `[${localTimestamp()}] bootstrap start pid=${process.pid}
`);
...
function writeBridgeState(active) {
  try {
    const stateFile = path2.join(os2.tmpdir(), "trib-channels", "bridge-state.json");
    fs2.mkdirSync(path2.dirname(stateFile), { recursive: true });
    fs2.writeFileSync(stateFile, JSON.stringify({ active, ts: Date.now() }));
  } catch {
  }
}
...
var TURN_END_BASENAME = path2.basename(TURN_END_FILE);
var TURN_END_DIR = path2.dirname(TURN_END_FILE);
removeFileIfExists(TURN_END_FILE);
var turnEndWatcher = fs2.watch(TURN_END_DIR, async (_event, filename) => {
  if (filename !== TURN_END_BASENAME) return;
  try {
    const stat = fs2.statSync(TURN_END_FILE);
    if (stat.size > 0) {
      stopServerTyping();
...
function sessionIdFromTranscriptPath(transcriptPath) {
  const base = path2.basename(transcriptPath);
...
try {
  controlWorker = spawn3(process.execPath, [path2.join(PLUGIN_ROOT, "hooks", "control-worker.cjs"), INSTANCE_ID], {
    stdio: "ignore",
    detached: false
  });
} catch (err) {
...
function applyTranscriptBinding(channelId, transcriptPath, options = {}) {
  ...
  void ingestTranscript(transcriptPath);
...
async function proxyRequest(endpoint, method, body) {
  return new Promise((resolve3) => {
    ...
    const req = http3.request(reqOpts, (res) => {
      let data = "";
      res.on("data", (chunk2) => {
        data += chunk2;
      });
...
function bindPersistedTranscriptIfAny() {
  ...
  if (!currentStatus.channelId) {
    try {
      const files = fs2.readdirSync(RUNTIME_ROOT).filter((f) => f.startsWith("status-") && f.endsWith(".json")).map((f) => {
        const full = path2.join(RUNTIME_ROOT, f);
        return { path: full, mtime: fs2.statSync(full).mtimeMs };
...
scheduler.setInjectHandler((channelId, name, prompt) => {
...
eventQueue.setInjectHandler((channelId, name, prompt) => {
  const ts = (/* @__PURE__ */ new Date()).toISOString();
  void mcp.notification({
    method: "notifications/claude/channel",
    params: {
    ...
    try {
      process.stderr.write(`trib-channels event: notification failed: ${e}
`);
    } catch {
    }
  });
...
backend.onInteraction = (interaction) => {
  if (interaction.customId?.startsWith("perm-")) {
    ...
    if (!access) {
      fs2.appendFileSync(_bootLog, `[${localTimestamp()}] perm interaction dropped: no access config
`);
      return;
    }
...
`);
    ...
    if (!fs2.existsSync(resultPath)) {
      fs2.writeFileSync(resultPath, action);
    }
...
async function findWhisper(override) {
  if (override) {
    if (override.includes(path2.sep) || override.includes("/")) {
...
function candidateModelDirs(whisperCmd) {
  ...
  const whisperDir = path2.dirname(whisperCmd);
  const dirs = [
    ...
    path2.join(DATA_DIR, "voice", "models"),
    path2.join(DATA_DIR, "models"),
    path2.join(process.cwd(), "models"),
    path2.join(path2.dirname(process.cwd()), "models"),
    path2.join(home, ".cache", "whisper"),
    path2.join(home, ".local", "share", "whisper"),
    path2.join(home, ".local", "share", "whisper.cpp", "models"),
    path2.join(home, "whisper.cpp", "models"),
    path2.join(whisperDir, "models"),
    path2.join(whisperDir, "..", "models"),
  ...
  if (process.platform === "win32") {
    dirs.push(
      path2.join(home, "AppData", "Local", "whisper"),
      path2.join(home, "AppData", "Local", "whisper.cpp", "models"),
      path2.join(home, "scoop", "persist", "whisper.cpp", "models")
    );
  }
  return dirs.filter((value) => Boolean(value)).map((value) => path2.resolve(value)).filter((value, index, arr) => arr.indexOf(value) === index);
}
async function findWhisperModel(override, whisperCmd) {
  if (override) {
    const resolvedOverride = path2.resolve(override);
...
var INBOUND_DEDUP_DIR = path2.join(os2.tmpdir(), "trib-channels-inbound");
ensureDir(INBOUND_DEDUP_DIR);
function claimChannelOwner(channelId) {
  const ownerPath = getChannelOwnerPath(channelId);
  try {
    fs2.writeFileSync(ownerPath, JSON.stringify({ instanceId: INSTANCE_ID, pid: process.pid, updatedAt: Date.now() }));
    return true;
  } catch {
    return false;
  }
...
function shouldDropDuplicateInbound(msg) {
  ...
  const marker = path2.join(INBOUND_DEDUP_DIR, key.replace(/:/g, "_"));
  try {
  ...
  if (Math.random() < 0.1) {
    try {
      for (const f of fs2.readdirSync(INBOUND_DEDUP_DIR)) {
        const fp = path2.join(INBOUND_DEDUP_DIR, f);
        try {
          if (now - fs2.statSync(fp).mtimeMs > INBOUND_DEDUP_TTL) removeFileIfExists(fp);
        } catch {
        }
      }
    } catch {
...
`);
  ...
  try {
    const memoryContext = await getHints(messageBody, {
...
`);
  ...
  const notificationContent = memoryContextBlock ? `${memoryContextBlock}
...
var _bootLog = path2.join(DATA_DIR, "boot.log");
fs2.appendFileSync(_bootLog, `[${localTimestamp()}] mcp.connect starting
`);
await mcp.connect(new StdioServerTransport());
fs2.appendFileSync(_bootLog, `[${localTimestamp()}] mcp.connect done
`);
...
var _channelFlagDetected = detectChannelFlag();
fs2.appendFileSync(_bootLog, `[${localTimestamp()}] channelFlag: ${_channelFlagDetected}
`);
if (_channelFlagDetected) {
  channelBridgeActive = true;
  fs2.appendFileSync(_bootLog, `[${localTimestamp()}] channel mode detected \u2014 bridge auto-activated
`);
}
writeBridgeState(channelBridgeActive);
var previousOwner = readActiveInstance();
...
if (bridgeRuntimeConnected && channelBridgeActive) {
  const greetingDone = path2.join(DATA_DIR, ".greeting-sent");
  const today = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
  const lastGreetDate = tryRead(greetingDone);
  if (lastGreetDate === today) {
  } else {
    void (async () => {
      fs2.writeFileSync(greetingDone, today);
...
function shutdown() {
  ...
  try {
    process.stderr.write("trib-channels: shutting down\n");
  } catch {
  }
...
process.stdin.on("end", () => {
  try {
    process.stderr.write("[trib-channels] stdin end, shutting down...\n");
  } catch {
  }
  shutdown();
});
process.stdin.on("close", () => {
  try {
    process.stderr.write("[trib-channels] stdin closed, shutting down...\n");
  } catch {
  }
  shutdown();
});
