...
try {
  ...
  // Write recent.md for session-start hook (file-based, no HTTP needed)
  try {
    const { getTagFactor } = await import('../lib/memory-score-utils.mjs')
    const parts = []
    // What User Asked — previous session user messages
    const latestRef = store.db.prepare(`SELECT source_ref FROM episodes WHERE source_ref LIKE 'transcript:%' ORDER BY id DESC LIMIT 1`).get()
    if (latestRef) {
      const currentSessionId = String(latestRef.source_ref).split(':')[1]
      const prevRef = store.db.prepare(`SELECT source_ref FROM episodes WHERE source_ref LIKE 'transcript:%' AND source_ref NOT LIKE ? ORDER BY id DESC LIMIT 1`).get(`transcript:${currentSessionId}:%`)
      if (prevRef) {
        const prevSessionId = String(prevRef.source_ref).split(':')[1]
        const userTurns = store.db.prepare(`SELECT content FROM episodes WHERE source_ref LIKE ? AND kind = 'message' AND role = 'user' AND content NOT LIKE 'You are%' AND LENGTH(content) >= 5 ORDER BY id ASC`).all(`transcript:${prevSessionId}:%`)
    ...
    // What Was Decided
    const recentDecisions = store.db.prepare(`SELECT id, topic, element, importance, episode_id FROM classifications WHERE status = 'active' AND importance IS NOT NULL AND importance != '' ORDER BY updated_at DESC LIMIT 50`).all().filter(row => getTagFactor(row.importance) < 1.0)
    ...
    if (keyItems.length > 0) {
      const lines = keyItems.map(row => {
        ...
        if (row.episode_id) {
          ...
          if (ep) content = String(ep.content).slice(0, 300)
        }
        return `- [${tag}] ${content}`
      })
    ...
    if (parts.length > 0) {
      fs.writeFileSync(path.join(DATA_DIR, 'history', 'recent.md'), parts.join('\n\n'))
    }
  } catch {}
