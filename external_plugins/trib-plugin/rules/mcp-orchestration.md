...
- All LLM calls converge through the MCP `mcp__plugin_trib-plugin_trib-plugin__bridge` tool. Session-based, parallel, requires `role` (primitive only, no suffixes); role resolves to a preset via `user-workflow.json`.
