...
- Exception: documentation-lookup agents (e.g. `claude-code-guide`) may be invoked directly via the `Agent` tool. These are read-only doc/reference queries, not work delegation, so they fall outside the bridge single-pathway rule.
