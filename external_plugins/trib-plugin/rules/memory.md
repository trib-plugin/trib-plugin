...
- **Retrieval order: recall (past context) → search (external web) → explore (codebase).** Never skip `recall` when past context may apply — cost is near zero, missing context is expensive.
