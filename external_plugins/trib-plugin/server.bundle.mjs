...
var MAX_CHUNK_LIMIT, MAX_ATTACHMENT_BYTES, RECENT_SENT_CAP, DiscordBackend;
...
var DATA_DIR, PLUGIN_ROOT, CONFIG_FILE, DEFAULT_ACCESS, DEFAULT_CONFIG, HEADLESS_BACKEND, BOT_FILE, PROFILE_FILE;
var init_config = __esm({
  "src/channels/lib/config.ts"() {
    ...
    DATA_DIR = process.env.CLAUDE_PLUGIN_DATA;
    PLUGIN_ROOT = process.env.CLAUDE_PLUGIN_ROOT ?? new URL("..", import.meta.url).pathname;
    CONFIG_FILE = join2(DATA_DIR, "config.json");
    DEFAULT_ACCESS = {
      dmPolicy: "pairing",
      allowFrom: [],
      channels: {}
    ...
    BOT_FILE = join2(DATA_DIR, "bot.json");
    PROFILE_FILE = join2(DATA_DIR, "profile.json");
  }
});
...
var init_cli_worker_host = __esm({
  "src/channels/lib/cli-worker-host.ts"() {
    "use strict";
  }
});
...
var SCRIPTS_DIR, NOPLUGIN_DIR, EVENT_LOG;
var init_executor = __esm({
  "src/channels/lib/executor.ts"() {
    "use strict";
    init_config();
    init_cli_worker_host();
    SCRIPTS_DIR = join3(DATA_DIR, "scripts");
    NOPLUGIN_DIR = join3(tmpdir(), "trib-plugin-noplugin");
    EVENT_LOG = join3(DATA_DIR, "event.log");
  }
});
...
var CACHE_FILE, FALLBACK_FILE, CACHE_MAX_AGE_MS;
var init_holidays = __esm({
  "src/channels/lib/holidays.ts"() {
    "use strict";
    init_config();
    CACHE_FILE = join4(DATA_DIR, "holidays-cache.json");
    FALLBACK_FILE = join4(homedir(), ".claude", "schedules", "holidays.json");
    CACHE_MAX_AGE_MS = 30 * 24 * 60 * 60 * 1e3;
  }
});
...
var DELEGATE_CLI, SCHEDULE_LOG, TICK_INTERVAL, FREQUENCY_MAP, Scheduler;
var init_scheduler = __esm({
  "src/channels/lib/scheduler.ts"() {
    ...
    init_holidays();
    init_settings();
    DELEGATE_CLI = join5(PLUGIN_ROOT, "scripts", "delegate-cli.mjs");
    SCHEDULE_LOG = join5(DATA_DIR, "schedule.log");
    TICK_INTERVAL = 6e4;
    FREQUENCY_MAP = {
...
var WEBHOOKS_DIR, DELEGATE_CLI2, WEBHOOK_LOG, SIGNATURE_HEADERS, NGROK_PID_FILE, WebhookServer;
var init_webhook = __esm({
  "src/channels/lib/webhook.ts"() {
    "use strict";
    init_config();
    WEBHOOKS_DIR = join6(DATA_DIR, "webhooks");
    DELEGATE_CLI2 = join6(PLUGIN_ROOT, "scripts", "delegate-cli.mjs");
    WEBHOOK_LOG = join6(DATA_DIR, "webhook.log");
    SIGNATURE_HEADERS = {
    ...
    NGROK_PID_FILE = join6(DATA_DIR, "ngrok.pid");
    WebhookServer = class {
      config;
      server = null;
      eventPipeline = null;
...
var QUEUE_DIR, PROCESSED_DIR, EventQueue;
var init_event_queue = __esm({
  "src/channels/lib/event-queue.ts"() {
    ...
    init_state_file();
    init_executor();
    QUEUE_DIR = join7(DATA_DIR, "events", "queue");
    PROCESSED_DIR = join7(DATA_DIR, "events", "processed");
    EventQueue = class {
      config;
      channelsConfig;
      tickTimer = null;
...
var OutputForwarder;
var init_output_forwarder = __esm({
  "src/channels/lib/output-forwarder.ts"() {
    "use strict";
    init_format();
    OutputForwarder = class _OutputForwarder {
...
var RUNTIME_ROOT, OWNER_DIR, ACTIVE_INSTANCE_FILE, RUNTIME_STALE_TTL, SERVER_PID_FILE;
var init_runtime_paths = __esm({
  "src/channels/lib/runtime-paths.ts"() {
    "use strict";
    init_state_file();
    RUNTIME_ROOT = join9(tmpdir3(), "trib-plugin");
    OWNER_DIR = join9(RUNTIME_ROOT, "owners");
    ACTIVE_INSTANCE_FILE = join9(RUNTIME_ROOT, "active-instance.json");
    RUNTIME_STALE_TTL = 24 * 60 * 60 * 1e3;
    SERVER_PID_FILE = join9(
      RUNTIME_ROOT,
...
var PendingInteractionStore;
var init_interaction_workflows = __esm({
  "src/channels/lib/interaction-workflows.ts"() {
    "use strict";
    PendingInteractionStore = class {
...
__export(channels_exports, {
  ...
  isChannelBridgeActive: () => isChannelBridgeActive,
  start: () => start,
  stop: () => stop
});
...
var memoryClientModulePath, memoryAppendEpisode, memoryIngestTranscript, getProactiveSources, getProactiveContext, applyProactiveUpdates, DEFAULT_PLUGIN_VERSION, PLUGIN_VERSION, crashLogging, _bootLogEarly, _bootLog, config, botConfig, backend, INSTANCE_ID, INSTRUCTIONS, mcpServer, channelBridgeActive, typingChannelId, pendingSetup, TURN_END_FILE, TURN_END_BASENAME, TURN_END_DIR, turnEndWatcher, STATUS_FILE, statusState, forwarder, scheduler, webhookServer, eventPipeline, bridgeRuntimeConnected, bridgeOwnershipRefreshRunning, bridgeOwnershipTimer, lastOwnershipNote, ACTIVE_OWNER_STALE_MS, proxyMode, ownerHttpPort, ownerHttpServer, PROXY_PORT_MIN, PROXY_PORT_MAX, eventQueue, resolvedWhisperCmd, resolvedWhisperModel, resolvedWhisperLanguage, resolvedWhisperType, whichCmd, TOOL_DEFS, BACKEND_TOOLS, INBOUND_DEDUP_TTL, inboundSeen, INBOUND_DEDUP_DIR, inboundQueue, detectChannelFlag2, shutdown2;
var init_channels = __esm({
  async "src/channels/index.ts"() {
    "use strict";
    init_config();
    init_settings();
    init_scheduler();
    ...
    init_event_pipeline();
    init_cli_worker_host();
    init_output_forwarder();
    init_session_control();
    init_state_file();
    init_interaction_workflows();
    init_runtime_paths();
    init_config();
    memoryClientModulePath = pathToFileURL(path.join(PLUGIN_ROOT, "src/channels/lib/memory-client.mjs")).href;
    ({
...
`);
    _bootLog = path.join(DATA_DIR, "boot.log");
    config = loadConfig();
    botConfig = loadBotConfig();
    ...
    typingChannelId = null;
    pendingSetup = new PendingInteractionStore();
    TURN_END_FILE = getTurnEndPath(INSTANCE_ID);
    TURN_END_BASENAME = path.basename(TURN_END_FILE);
    TURN_END_DIR = path.dirname(TURN_END_FILE);
    ...
    STATUS_FILE = getStatusPath(INSTANCE_ID);
    statusState = new JsonStateFile(STATUS_FILE, {});
    statusState.ensure();
    forwarder = new OutputForwarder({
      send: async (ch, text) => {
        if (!channelBridgeActive) return;
...
`);
    ...
    wireWebhookHandlers();
    eventQueue = eventPipeline.getQueue();
    eventQueue.setInjectHandler((channelId, name, content, options) => {
      const ts = (/* @__PURE__ */ new Date()).toISOString();
      const now = /* @__PURE__ */ new Date();
      const timeLabel = `${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")} `;
...
`);
    ...
    resolvedWhisperCmd = null;
    resolvedWhisperModel = null;
    resolvedWhisperLanguage = null;
    resolvedWhisperType = null;
    whichCmd = process.platform === "win32" ? "where" : "which";
    TOOL_DEFS = [
      {
        name: "reply",
        title: "Discord Reply",
    ...
    BACKEND_TOOLS = /* @__PURE__ */ new Set(["reply", "fetch", "react", "edit_message", "download_attachment"]);
    ...
    INBOUND_DEDUP_TTL = 5 * 6e4;
    inboundSeen = /* @__PURE__ */ new Map();
    INBOUND_DEDUP_DIR = path.join(os.tmpdir(), "trib-plugin-inbound");
...
async function setup(server, resolveReady) {
  process.stderr.write(`[trib-plugin] loading modules...
`);
  const channelsMod = await init_channels().then(() => channels_exports);
  const CHANNELS_TOOLS = channelsMod.TOOL_DEFS;
  ...
  const searchModulePath = pathToFileURL2(join11(PLUGIN_ROOT2, "src/search/index.mjs")).href;
  const searchMod = await import(searchModulePath);
  const SEARCH_TOOLS = searchMod.TOOL_DEFS;
  const searchHandleToolCall = searchMod.handleToolCall;
  const searchStart = searchMod.start;
  const searchStop = searchMod.stop;
  const agentModulePath = pathToFileURL2(join11(PLUGIN_ROOT2, "src/agent/index.mjs")).href;
  const agentMod = await import(agentModulePath);
  const AGENT_TOOLS = agentMod.TOOL_DEFS;
  ...
  const memoryModulePath = pathToFileURL2(join11(PLUGIN_ROOT2, "src/memory/index.mjs")).href;
  const memoryMod = await import(memoryModulePath);
  const MEMORY_TOOLS = memoryMod.TOOL_DEFS;
  ...
  const CHANNELS_TOOL_NAMES = new Set(CHANNELS_TOOLS.map((t) => t.name));
  ...
  ;
  globalThis.__tribTools = ALL_TOOLS;
  globalThis.__tribHandleToolCall = async (request3, srv) => {
    ...
    switch (module) {
      ...
      case "agent":
        return await agentHandleToolCall(name, toolArgs, {
          ...
          elicitFn: (opts) => srv.elicitInput?.(opts)
        });
      case "memory":
  ...
  globalThis.__tribShutdown = async () => {
    try {
      await channelsStop();
    } catch {
    }
...
var isFastEntry = !!globalThis.__tribFastEntry;
if (!isFastEntry) {
  ...
  server.setRequestHandler(CallToolRequestSchema2, async (request3) => {
    ...
    return handler(request3, server);
  });
  process.stderr.write(`[trib-plugin] standalone server starting (v${PLUGIN_VERSION2})
`);
  ...
  async function shutdown(reason) {
    ...
    isShuttingDown = true;
    try {
      process.stderr.write(`[trib-plugin] ${reason || "shutting down"}
`);
    } catch {
    }
